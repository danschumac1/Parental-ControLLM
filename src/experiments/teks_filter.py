...
TO RUN: TRANSFORMERS BACKEND ########################################
    ...
    nohup env CUDA_VISIBLE_DEVICES=1 \
        ...
        --backend transformers \
        --model google/gemma-4-31B-it \
        > logs/gemma_transformers_teks_filter.log 2>&1 &

    tail -f logs/gemma_transformers_teks_filter.log
...
from utils.file_io import load_tsv_file, load_single_yaml_prompt, append_jsonl_file, save_args_as_config_json
from utils.prompting import generate
from utils.evaluation import evaluate_generation, evaluate_dataset, print_evaluation, save_evaluation
...
def build_prompt(row: dict, teks_df: pd.DataFrame, prompt_template: dict) -> list[dict]:
    ...
    blocked_cats = {
        x.strip()
        for x in str(row.get("blocked_cats", "") or "").split(",")
        if x.strip()
    }
    ...
    allowed_teks = teks_df.loc[
        (teks_df["grade_code"] <= pretend_age) & teks_df["module_code"].isin(allowed_cats),
        "expectation",
    ].tolist()

    disallowed_teks = teks_df.loc[
        teks_df["module_code"].isin(blocked_cats) | (teks_df["grade_code"] > pretend_age),
        "expectation",
    ].tolist()

    allowed_text = "\n".join(f"- {expectation}" for expectation in allowed_teks) or "None"
    disallowed_text = "\n".join(f"- {expectation}" for expectation in disallowed_teks) or "None"

    system_prompt = prompt_template["system_prompt"].format(
        allowed_teks=allowed_text,
        disallowed_teks=disallowed_text,
    )

    user_prompt = prompt_template["user_prompt"].format(question=row["situation"])
...
def build_messages(data: list[dict], teks_df: pd.DataFrame, prompt_template: dict) -> list[list[dict]]:
...
def prepare_run_paths(args: argparse.Namespace) -> tuple[str, str, str]:
    ...
    return run_dir, output_path, eval_path


def get_completed_outputs(data: list[dict], output_path: str) -> tuple[dict[str, dict], list[dict]]:
    existing_outputs = load_existing_outputs(output_path)
    selected_ids = {row["id"] for row in data}
    ...
    return completed_outputs, remaining_data


def build_current_output(data: list[dict], completed_outputs: dict[str, dict]) -> list[dict]:
    return [
...
def main():
    ...
    run_dir, output_path, eval_path = prepare_run_paths(args)
    save_args_as_config_json(args, output_path)

    completed_outputs, remaining_data = get_completed_outputs(data, output_path)
    ...
    for batch_start in range(0, len(remaining_data), args.save_every):
        ...
        if not example_printed:
            example_messages = build_messages(batch_data, teks_df, prompt_template)
        ...
        current_output = build_current_output(data, completed_outputs)
    ...
    final_output = build_current_output(data, completed_outputs)
